...
def prime_check(number: int) ->bool:
    """checks to see if a number is prime in O(sqrt(n)).
...
class Test(unittest.Testcase):
    ...
    def test_not_primes(self):
        self.assertFalse(
            prime_check(-19),
            "Negative numbers are excluded by defination of prime numbers.",
        )  
        self.assertFalse(
            prime_check(0),
            "Zero dosen`t have any positive factors, primes must have exactly two.",
        )
        self.assertFalse(
            prime_check(1),
